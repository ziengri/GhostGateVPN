...
async def test_role_dependency_accepts_only_configured_roles() -> None:
    ...
    user = User(email="user@example.com", password_hash="x", role=UserRole.user)
...
def test_config_read_access_contract() -> None:
    ...
    owner = User(id=owner_id, email="owner@example.com", password_hash="x", role=UserRole.user)
    other_user = User(id=other_id, email="other@example.com", password_hash="x", role=UserRole.user)
    support = User(id=uuid.uuid4(), email="support@example.com", password_hash="x", role=UserRole.support)
    admin = User(id=uuid.uuid4(), email="admin@example.com", password_hash="x", role=UserRole.admin)
